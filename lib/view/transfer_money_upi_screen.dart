...
class TransferMoneyUPIScreen extends StatefulWidget {
  final BeneficiaryUPIData beneficiary;

  const TransferMoneyUPIScreen({
    super.key,
    required this.beneficiary,
  });

  @override
  State<TransferMoneyUPIScreen> createState() => _TransferMoneyUPIScreenState();
}

class _TransferMoneyUPIScreenState extends State<TransferMoneyUPIScreen> {
  ...
  void initState() {
    ...
    upiWalletController.transferAmountController.value.clear();
    upiWalletController.transferConfirmAmountController.value.clear();
    // dmtController.tpinController.value.clear();

    // Add listeners for amount fields
    upiWalletController.transferAmountController.value.addListener(() {
      amountInWords.value = convertToWords(
          upiWalletController.transferAmountController.value.text.trim()
      );
    });

    upiWalletController.transferConfirmAmountController.value.addListener(() {
      confirmAmountInWords.value = convertToWords(
          upiWalletController.transferConfirmAmountController.value.text.trim()
      );
    });
  }
      ...
      child: Column(
        ...
        children: [
          ...
          GlobalUtils.CustomTextField(
            label: "Amount",
            showLabel: false,
            controller: upiWalletController.transferAmountController.value,
            placeholder: "Amount",
            height: GlobalUtils.screenWidth * (60 / 393),
            width: GlobalUtils.screenWidth * 0.9,
            backgroundColor: Colors.white,
          ...
          GlobalUtils.CustomTextField(
            ...
            showLabel: false,
            controller: upiWalletController.transferConfirmAmountController.value,
  ...
  Widget buildQuickAmountButton(String amount, String confirmAmount) {
    return Expanded(
      child: GestureDetector(
        onTap: () {
          upiWalletController.transferAmountController.value.text = amount;
          upiWalletController.transferConfirmAmountController.value.text = confirmAmount;
          // setState(() {
          //   showTPINField = true;
          // });
        },
  ...
  Widget buildTransferButton(BuildContext transferMoneyContext) {
    return GlobalUtils.CustomButton(
      ...
      text: "Transfer",
      onPressed: () async {
        String amount = upiWalletController.transferAmountController.value.text.trim();
        String confirmAmount = upiWalletController.transferConfirmAmountController.value.text.trim();
